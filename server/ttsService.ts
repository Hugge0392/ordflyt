...
export class TextToSpeechService {
  ...
  constructor() {
    ...
    if (!subscriptionKey || !serviceRegion) {
      console.warn(
        `⚠️  TTS SERVICE WARNING: Missing Azure Speech credentials (AZURE_SPEECH_KEY=${!!subscriptionKey}, AZURE_SPEECH_REGION=${!!serviceRegion}). ` +
        'Text-to-speech will be disabled but app will continue to work in development mode.'
      );
    ...
    console.log('✅ Text-to-speech service configured with Azure');
  }

  async synthesizeSpeech(request: TTSRequest): Promise<Buffer> {
    if (!this.isConfigured || !this.speechConfig) {
      console.log('📢 TTS SIMULATION: Would synthesize speech for text:', request.text.substring(0, 50));
      return Buffer.from([]); // Return empty buffer in development
    }
